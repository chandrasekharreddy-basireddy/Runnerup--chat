...
class Message(Base):
    ...
    created_at: Mapped[dt.datetime] = mapped_column(TS, server_default=func.now())
    # Generated by PostgreSQL, never written by the application. Declared with Computed
    # so SQLAlchemy excludes it from INSERT and UPDATE statements — and so that
    # `Message.search_tsv` exists as a queryable attribute for the search path.
